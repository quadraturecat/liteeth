...
"""10GBASE-R and 5GBASE-R over Kintex-7 GTX transceivers.

Uses the same vendored BASE-R PCS as the Artix-7 GTP PHY (liteeth.phy.baser), with the GTX
native 64B/66B gearbox doing the framing.
"""
...
class K7_GTX_10G_BASER(LiteXModule):
    """10GBASE-R via a Kintex-7 GTX transceiver, driven from a shared GTXQuadPLL."""
    dw          = 64
    linerate    = 10.3125e9
    ...
    def __init__(self, qpll, data_pads, sys_clk_freq, platform,
        ...
        # Define the transceiver output clocks up front. Vivado reads XDC sequentially, so this
        # has to precede any constraint that resolves a clock through them.
        # TX/RXOUTCLK (RXOUTCLKPMA) run at the internal 4-byte rate, linerate/32.
        raw_clk_period = f"{1e9/(self.linerate/32):.3f}"
        platform.add_platform_command(
            "create_clock -name {txoutclk} -period " + raw_clk_period + " [get_nets {txoutclk}]",
            txoutclk=self.txoutclk)
        platform.add_platform_command(
            "create_clock -name {rxoutclk} -period " + raw_clk_period + " [get_nets {rxoutclk}]",
            rxoutclk=self.rxoutclk)

        # Diagnostics, exposed so a probe can be built outside this module.
        self.tx_reset_done     = Signal()
        self.rx_reset_done     = Signal()
        self.qpll_lock         = qpll.lock
        self.tx_mmcm_locked    = Signal()
        self.rx_mmcm_locked    = Signal()

        # # #

        # PCS. The gated clocks tick once per 66-bit block.
        self.tx_pcs_clk = tx_pcs_clk = Signal()
        self.rx_pcs_clk = rx_pcs_clk = Signal()
        self.pcs = pcs = LiteEthPHYBaseRPCS(
            ...
            clk_freq = self.linerate/66,
        )

        # Block-boundary enables for XGMII wiring.
        # Driven in the TX gearbox and RX sections further down.
        tx_gearbox_ready = Signal()
        rx_block_ce      = Signal()

        # XGMII.
        self.xgmii_pads = xgmii_pads = LiteEthPHYXGMIIPads()
        # CEInserter() is what provides the .ce used below to advance these one word per block.
        ...
        # Pipeline the MAC -> XGMII handoff. At 10GBASE-R this domain runs at 161.13MHz and the
        # MAC's TX CRC to the XGMII adapter is the critical path. Increases latency by one cycle,
        # needed for timing.
        # Must live in eth_tx like xgmii_tx: a bare stream.Buffer lands in sys and silently
        # creates an unsynchronised sys <-> eth_tx crossing.
        ...
        self.comb += [
            ...
            self.xgmii_tx.sink.valid.eq(tx_pipe.source.valid & tx_gearbox_ready),
            tx_pipe.source.ready.eq(self.xgmii_tx.sink.ready & tx_gearbox_ready),
            ...
            self.xgmii_tx.ce.eq(tx_gearbox_ready),
            self.xgmii_rx.ce.eq(rx_block_ce),
        ]

        # Delay the MAC-facing valid by one cycle to match the enable, as the A7 PHY does.
        rx_mac_ce = Signal()
        ...
        # TX gearbox sequencing. With the external sequence counter the fabric controls the pause:
        # 33 cycles carry 32 blocks (33*64 = 2112 = 32*66), so TXSEQUENCE 0..31 present payload
        # and 32 is the pause. TXGEARBOXREADY is not a flow-control input in this mode.
        self.sync.eth_tx += [
            If(tx_sequence == 32,
                tx_sequence.eq(0),
            ).Else(
        ...
        self.comb += [
            tx_gearbox_ready.eq(tx_sequence != 32),
            tx_data.eq(pcs.serdes_tx_data),
            tx_header.eq(Cat(pcs.serdes_tx_hdr, 0)),
        ]
        # Exposed for a TX-domain probe.
        self.tx_sequence      = tx_sequence
        self.tx_gearbox_ready = tx_gearbox_ready
        self.tx_header        = tx_header
        self.tx_data          = tx_data

        # RX: one 64-bit block per RXUSRCLK2 cycle, straight into the PCS.
        self.comb += [
            rx_block_ce.eq(rx_data_valid[0]),
            pcs.serdes_rx_data.eq(rx_data),
            pcs.serdes_rx_hdr.eq(rx_header[:2]),
        ]
        ...
        # UG476: RXGEARBOXSLIP must be asserted for exactly one RXUSRCLK2 cycle and then held
        # low for at least 32 cycles while the gearbox settles.
        # Hold off for 64 cycles after each accepted slip.
        SLIP_HOLDOFF  = 64
        rx_slip_d     = Signal()
        ...
        self.comb += rx_slip_pulse.eq((rx_slip_wait == 0) & pcs.serdes_rx_slip & ~rx_slip_d)
        self.rx_slip_pulse = rx_slip_pulse

        # Gated PCS clocks, one tick per block.
        # Gate the *buffered* fabric clock, not the raw MMCM output.
        self.specials += [
            Instance("BUFGCE", p_SIM_DEVICE="7SERIES", i_I=self.cd_eth_tx.clk,
                     i_CE=tx_gearbox_ready | ResetSignal("eth_tx"), o_O=tx_pcs_clk),
            Instance("BUFGCE", p_SIM_DEVICE="7SERIES", i_I=self.cd_eth_rx.clk,
                     i_CE=rx_block_ce | ResetSignal("eth_rx"), o_O=rx_pcs_clk),
        ]
        ...
        self.sync += tx_mmcm_reset.eq(~qpll.lock)
        ...
        self.comb += [
            ps_restart.i.eq(pcs.rx_reset_req),
        ...
        # NOTE: the multicycle constraints for the gated PCS clocks are emitted by the SoC via
        # add_timing_constraints(), after add_etherbone() has defined the eth clocks.

        if with_csr:
            self.add_csr()

    def add_timing_constraints(self, platform):
        """No timing exceptions are needed for the gated PCS clocks on this datapath."""
        pass
...
class K7_GTX_5G_BASER(K7_GTX_10G_BASER):
    ...
    Identical to the 10G chain at half the rate: the QPLL keeps its 10.3125GHz VCO and the
    channel output divider goes from d=1 to d=2, so every user clock halves.
    """
    linerate    = 5.15625e9
    rx_clk_freq = linerate/64
    tx_clk_freq = linerate/64
